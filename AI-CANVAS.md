...
> Restored 7 Sep 2026. This file was deleted in `2f9303d`, the commit that
> renamed `nightly/` to `night_agent/`, without being mentioned in the message —
> which explains every other deletion in that commit at length. Twelve places in
> the repo still cite it as the live document, by section and by step number, and
> the README kept only a summary of it. So the deletion reads as collateral
> rather than a decision, and the file is back exactly as it was, with one path
> corrected for the folder moves since.
...
- The skill, `agents/pa_agent/skills/pa-attach/`: thin, as promised — `scripts/attach_session.py`
