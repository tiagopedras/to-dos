...
if (!process.env.BOARD_PORT) console.error('Note: this runs against the live board on 8765. For a throwaway copy: scripts/test-board.sh test_matrix.mjs')
const checks = []
...
const chrome = spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', [
  ...
  `--user-data-dir=${process.env.CHROME_PROFILE || '/tmp/todo-matrix-test-profile'}`, '--window-size=1400,1000',
...
check('and says how many it took off', await evalJS(`
  /1 hidden, sitting in Reviewing/.test(document.querySelector('.mhidden')?.textContent || '')
