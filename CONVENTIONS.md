...
Inside each bucket, five states, Done first in the file because the board draws a bucket's headings the other way round. Doing / To do / Backlog replaced Now / Next / Later / Parked on 10 Aug 2026 at his request; if a file still uses the old four, rename the headings rather than adding new ones alongside. Reviewing was added on 26 Aug 2026 as Waiting review, renamed Waiting for review on 17 Sep 2026 and Reviewing on 21 Sep 2026; the readers still accept both old headings. Blocked went on 21 Sep 2026: a column says the state of the card, and its tasks moved to Reviewing.

- **Done** — where a ticked task lives, the first heading in every bucket and the far right of the board. It has been a heading since 21 Sep 2026; before that it was only the tick. The two go together: ticking a task moves it under Done (newest first), and unticking one there sends it back to To do. Every reader treats a ticked task under any other heading as if it were under Done, so a tick made in place is read correctly and the board writes it under Done on its next save. When you tick a task, move it there in the same edit.
- **Reviewing** — finished as far as he is concerned, sitting with somebody else for sign-off. On the board it is the last column before Done, so in the file it is the heading right after Done. A task waiting on a person or an event gets a `Waiting on:` note here; a task waiting on another task uses `blocked-by:` instead, wherever it actually sits.
...
**Nothing goes into Reviewing on his behalf either.** Like Doing, it is a statement of fact about where the work sits, and only he knows whether the pack actually went. Move a task there when he says he has sent it, handed it over or is waiting on a reply; move it back to Doing when the review comes back with work in it, and tick it when it comes back clear. A task sitting in Reviewing with no note saying who has it is worth asking about. Waiting on a person is the state most likely to be forgotten.
...
- **Ticked and the date passed, with the next occurrence under a week away** — the prep was delivered, so the agenda is filed as `Previous agenda (that date):`, and the task is parked out of wherever it was sitting: To do if the next occurrence is under two days away, Backlog if it is two days or more. A card parked in Backlog is brought across to To do on a later load, once its day is under two days off.
- **Ticked and the date passed, with a week or more to the next occurrence** — nothing happens at all. The card keeps its date, its tick, its agenda and its place in Done, because a monthly meeting already prepared for has nothing waiting on it for weeks. The roll fires on a later load once that week has closed, and until then the card carries a date that has gone and reads as prepared for an occurrence that is over.
- **Unticked and the date passed** — the prep never happened, so those topics were never raised. The agenda **carries forward** onto the new date instead of being archived, and the card is left exactly where it was: only a “done” carries an opinion about what comes next.
