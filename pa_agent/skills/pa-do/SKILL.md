...
by `is_stale()` in `night_agent/pick.py`. While a plan sits at `agreed` the nightly
...
## Move 3: hand it to `pa-execute`

One `pa-execute` run per plan. Give it:
...
`pa-execute` reports what it did, what it left and what needs him. Pass that on
...
If it asked for a change to `todo.md`, that change is his to approve line by
line. `pa-execute` is the only agent allowed to write that file and it has to
show the before and after first. Tell him to press **Reload** on the board
afterwards rather than save, in that order: a tab open since before the write is
holding a stale document, and a save from it would undo the change.
