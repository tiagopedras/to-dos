#!/bin/bash
# Packages every pa-* skill in this folder into pa_agent/dist/<name>.skill.
...
ROOT="$(cd .. && pwd)"
DIST="$(pwd)/dist"
# The skills live one level down, in skills/, so the packaging script and its
# dist/ folder sit beside them rather than among them.
SKILLS="$(pwd)/skills"
STAGE="$(mktemp -d)"
...
echo
echo "Written to pa_agent/dist/. Install one by dragging it onto Claude, or with"
