...
growth conversations. Read `agents/night_agent/PLAN-BRIEF.md` in this repo first — it holds
...
`~/Code/TWINKL/twinkl-hr/` is where the processes themselves are written down, one folder
per process, all reading the same career framework. Read the folder for the process
your task touches rather than describing it from memory.

The gaps are real and each has a task on the list: scoring a filled-in interview
note against the JD, a `promotion-review` skill, a 1:1 evidence pull out of Jamie,
a performance snapshot skill, and a full `performance-review` skill that is blocked
until HR sends the guidance and a worked example. Proposing to build one of those is
fine. Proposing to build one of the four above is not.
